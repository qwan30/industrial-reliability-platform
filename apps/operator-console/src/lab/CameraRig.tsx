...
const HOME_POSITION = new THREE.Vector3(12, 10, 14);
...
export function CameraRig() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.code === "KeyH") {
        ...
        camera.position.set(focusTarget[0] + 3, focusTarget[1] + 2, focusTarget[2] + 4);
        controlsRef.current?.update();
      }
    };
  ...
  useEffect(() => {
    if (focusTarget && controlsRef.current) {
      controlsRef.current.target.set(focusTarget[0], focusTarget[1], focusTarget[2]);
      camera.position.set(focusTarget[0] + 3.5, focusTarget[1] + 2.5, focusTarget[2] + 4.5);
      controlsRef.current.update();
    }
  ...
  useFrame((_, delta) => {
    ...
    const speed = (keysDown.current["ShiftLeft"] || keysDown.current["ShiftRight"]) ? 6.0 : 4.0;
    ...
    if (keysDown.current["KeyW"] || keysDown.current["ArrowUp"]) moveVector.add(forward);
    if (keysDown.current["KeyS"] || keysDown.current["ArrowDown"]) moveVector.sub(forward);
    if (keysDown.current["KeyD"] || keysDown.current["ArrowRight"]) moveVector.add(right);
    if (keysDown.current["KeyA"] || keysDown.current["ArrowLeft"]) moveVector.sub(right);
