...
if ($CheckPreflight) {
    Write-Host "[1/6] Running host environment preflight checks..." -ForegroundColor Green
    python deploy/preflight.py
    if ($LASTEXITCODE -ne 0) {
        Write-Error "Preflight environment check failed. Resolve errors before running the demo."
    }
}

# 2. Resolve Git SHA
$sha = (git rev-parse HEAD).Trim().ToLowerInvariant()
Write-Host "[2/6] Current Git SHA: $sha" -ForegroundColor Green

# 3. Verify / Build Research Package
Write-Host "[3/6] Verifying research scoring candidate package..." -ForegroundColor Green
...
# 4. Execute Phase 8 Fault Isolation Drills
Write-Host "[4/6] Running Phase 8 fault isolation drills..." -ForegroundColor Green
.\scripts\run_phase8_live_fault_drills.ps1

# 5. Execute Phase 9 Grounded RCA Gate
Write-Host "[5/6] Running Phase 9 grounded RCA gate..." -ForegroundColor Green
.\scripts\run_phase9_live_gate.ps1

# 6. Execute Exact-SHA Release Certification
Write-Host "[6/6] Running exact-SHA release certification..." -ForegroundColor Green
...
Write-Host "  - Operator Console:    http://127.0.0.1:3000"
