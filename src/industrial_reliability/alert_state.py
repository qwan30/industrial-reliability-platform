"""Deterministic alert lifecycle state machine."""
...
class OrderingViolationError(ValueError):
    """Raised when decisions arrive with non-increasing source timestamps."""
...
class AlertState:
    ...
    @classmethod
    def empty(cls, session_id: UUID, machine_id: str) -> AlertState:
        return cls(
...
def alert_id_for(session_id: UUID, first_decision_id: UUID) -> UUID:
    return uuid5(NAMESPACE_URL, f"alert:{session_id}:{first_decision_id}")


def evidence_id_for(alert_id: UUID, decision_id: UUID) -> UUID:
    return uuid5(NAMESPACE_URL, f"evidence:{alert_id}:{decision_id}")
...
def _build_evidence_snapshot(
    ...
    if not deviations:
        deviations = (
            FeatureDeviationV1(
                feature_name="score",
                observed_value=decision.score,
                baseline_value=decision.threshold,
                absolute_deviation=abs(decision.score - decision.threshold),
            ),
        )
        ...
        if state.active_alert_id is None:
            if new_anomaly_streak >= policy.persistence_decisions:
                ...
                if (
                    ...
                    action = "REOPENED"
                    alert_id = state.previous_alert_id
                    first_det = state.first_detection or decision.source_timestamp
                else:
                    action = "OPENED"
                    alert_id = alert_id_for(decision.replay_session_id, new_anomaly_decision_ids[0])
                    first_det = decision.source_timestamp
            ...
            # Already active alert: emit UPDATED event
